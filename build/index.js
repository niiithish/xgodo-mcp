...
import { z } from "zod";
import { validateDocumentPath, getAllMarkdownFiles, readDocumentContent, getDataDir, } from "./utils.js";
...
server.registerTool("xgodo_list_documents", {
  ...
  JSON format: { "total": number, "documents": string[] }`,
    ...
    try {
        ...
        if (documents.length === 0) {
            const output = { total: 0, documents: [] };
            return {
                content: [
                    {
                        type: "text",
        ...
        const output = { total: documents.length, documents };
...
server.registerTool("xgodo_read_document", {
    title: "Read Document",
...
server.registerTool("xgodo_search_documents", {
...
Finds all documents containing the query and returns matching lines.`,
    ...
    try {
        ...
        if (results.length === 0) {
            const output = { query, totalMatches: 0, results: [] };
        ...
        const output = { query, totalMatches, results };
        if (response_format === ResponseFormat.JSON) {
            return {
                content: [{ type: "text", text: JSON.stringify(output, null, 2) }],
                structuredContent: { success: true, ...output },
...
server.registerTool("xgodo_list_agent_actions", {
...
server.registerTool("xgodo_list_agent_utils", {
...
server.registerTool("xgodo_get_types", {
