...
declare global {
  ...
  function getAllNodes(
    screenContent: AndroidNode | null | undefined,
  ): AndroidNode[];

  /** Build the path from root to target node */
  function buildNodePath(
    rootNode: AndroidNode | null | undefined,
    targetNode: AndroidNode | null | undefined,
  ): AndroidNode[];
  ...
  interface AgentActions {
    ...
    launchIntent(
      ...
      extras: object | null,
      flags: number,
      component: "activity" | "service" | "broadcast",
      isDataLocal?: boolean,
    ): Promise<void>;
    /** List installed apps */
    ...
    screenContent(): Promise<ScreenContent>;
    /** Get screen content from all displays */
    allScreensContent(): Promise<ScreenContent[]>;
    /** Take a screenshot */
    screenshot(
      maxWidth: number,
      maxHeight: number,
    ...
    // --- ADB Operations ---
    /** ADB interface for low-level device interaction */
    adb: {
  ...
  interface AgentUtils {
    ...
    isServerReachable(): Promise<
      { reachable: true } | { reachable: false; error: string }
    >;
    /** Wait for a node to appear on screen */
    waitForNode(
      condition: (node: AndroidNode) => boolean,
      durationMs?: number,
      intervalMs?: number,
    ): Promise<boolean>;
    ...
    setNetworkCallback(
      callback: ((networkAvailable: boolean) => void) | null,
    ): void;
    /** Toast callback */
    toastCallback:
      | ((packageName: string, data: { message: string }) => void)
      | null;
  ...
  interface OutOfStepsUtils {
    ...
    submit(
      ...
      { success: false; error: string } | { success: true; id: string }
    >;
  }
  ...
  interface AndroidNode {
    ...
    /** @deprecated use isClickable property instead */
    clickable?: boolean;
    /** @deprecated use isSelected property instead */
    selected?: boolean | number;
  }

  /** Screen content interface for querying the UI tree */
  interface ScreenContent extends AndroidNode {
    find(predicate: (node: AndroidNode) => boolean): AndroidNode | undefined;
    findAdvanced(
      filterFn: (filter: FilterBuilder) => FilterBuilder | boolean,
    ): AndroidNode | undefined;
    filterAdvanced(
      filterFn: (filter: FilterBuilder) => FilterBuilder | boolean,
    ): AndroidNode[];
